...
def  findAngle(img, p1, p2, p3, lmList, draw = True):
   ...
   # aç hesaplama
    ...
    if draw: 
        ...
        cv2.putText(img, str(int(angle)), ( x2 - 40 , y2 -14), cv2.FONT_HERSHEY_PLAIN, 2, (0,255, 255), 2)
        return angle 
...
while True:
    ...
    if len(lmList)!= 0:
        ...
        if per ==  100:
            if dir == 0:      
               count += 0.5
               dir = 1        # hareketin tam yapılıp yapılmadığının kontrolu
        ...
        cv2.putText(img, str(int(count)), (45,125), cv2.FONT_HERSHEY_PLAIN, 5, (255,0,0), 10)
